...
if ($results.psobject.properties.value -contains $false) {
    ...
    InformIT -message "The device $ENV:COMPUTERNAME is NOT compatible to run Windows 11, $results" -color "d73b00"
}
else {
    Write-Host "This device is compatible with Windows 11" -ForegroundColor Green
    $Results | Format-List | Out-File C:\Windows\Logs\Windows11Readiness.log

    InformIT -message "The device $ENV:COMPUTERNAME is compatible to run Windows 11, $results" -color "0bd700"
}